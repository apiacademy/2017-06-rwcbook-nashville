...
function getQueries(obj) {
  var data, d, f, query, q, rtn, i, x, j, y;
    ...
    for(i=0,x=obj.length;i<x;i++) {
        ...
        query.target.indexOf("cj") !==-1)
      {
        ...
        for(j=0,y=query.inputs.length;j<y;j++) {
          d = query.inputs[j];
          f = {};
          f.name = d.name||"input"+j;
          f.value = d.value||"",
          f.prompt = d.prompt||f.name;
          f.type = d.type||"text";
          if(d.required){f.required=d.required.toString();}
          if(d.readOnly){f.readOnly=d.readOnly.toString();}
          if(d.pattern){f.pattern=d.pattern;}
          if(d.min){f.min=d.min;}
          if(d.max){f.max=d.max;}
          if(d.maxlength){f.maxlength=d.maxlength;}
          if(d.size){f.size=d.size;}
          if(d.step){f.step=d.step;}
          if(d.cols){f.cols=d.cols;}
          if(d.rows){f.rows=d.rows;}
          if(d.suggest){f.suggest=d.suggest;}
          data.push(f);
        }
        q.data = data;
        rtn.push(q);
    ...
    for(i=0,x=obj.length;i<x;i++) {
      if(obj[i].target.indexOf("cj-template")!==-1) {
        ...
        for(j=0,y=temp.inputs.length;j<y;j++) {
          ...
          if(d.required){field.required=d.required.toString();}
          ...
          if(d.suggest){field.suggest=d.suggest;}
          data.push(field);
        }
      }
    }
